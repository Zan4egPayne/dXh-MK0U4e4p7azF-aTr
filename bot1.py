...
async def help( ctx, amount = 1 ):
    ...
    emb.add_field( name = '``{}balance``'.format( PREFIX ), value= 'Узнать Баланс.' )
    emb.add_field( name = '``{}award``'.format( PREFIX ), value= 'Выдать деньги.' )
    emb.add_field( name = '``{}take``'.format( PREFIX ), value= 'Забрать деньги.' )
    emb.add_field( name = '``{}add-shop``'.format( PREFIX ), value= 'Добавить роль в магазин.' )
    emb.add_field( name = '``{}remove-shop``'.format( PREFIX ), value= 'Удалить роль с магазина.' )
    emb.add_field( name = '``{}shop``'.format( PREFIX ), value= 'Магазин.' )
    emb.add_field( name = '``{}buy``'.format( PREFIX ), value= 'Купить.' )
    emb.add_field( name = '``{}rep``'.format( PREFIX ), value= 'Добавить репутацию пользователю.' )
    emb.add_field( name = '``{}lb``'.format( PREFIX ), value= '10 лидеров сервера.' )
    emb.set_footer( text = 'Всего команд: 26' )
...
Bot.run('NzQzMDY5Njc2ODQxNDY3OTE0.XzPTkA.PHflKVkh5nMkCJ2EFAglI_NbXjg')
